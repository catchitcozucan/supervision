...
public class UserService implements UserDetailsService {
    ...
    public UserAndHashedPassword createPassword(String user, String pwdInput, HttpServletRequest request, HttpServletResponse response) {
        ...
        UserAndHashedPassword userAndPwd = UserAndHashedPassword.builder()
                .userName(user)
                .hash(pwdInput).build();
        userAndPwd.validate();
        ...
        if (!user.equals(ADMIN1)) {
            ...
            loginRepository.save(LoginEntity.builder()
                    .pwdHash(hash)
                    .username(userAndPwd.getUsername())
                    .build());
    ...
    public UserAndHashedPassword verifyLogin(String user, String passwd, HttpServletRequest request, HttpServletResponse response) {
        ...
        if (logins.isEmpty()) {
            ...
            if (argo2Utility.matches(passwd, pwdHash)) {
                ...
                return UserAndHashedPassword.builder()
                        .userName(user)
                        .hash(YUP_THERE_IS_ONE).build();
            } else {
    ...
    private boolean adminIsAuthenticated() {
        return thereIsAnAuthenticatedUser() &&
                SecurityContextHolder.getContext().getAuthentication().getPrincipal().equals(ADMIN1);
    }
}

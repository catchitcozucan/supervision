...
public class SourceResponseResultConversionService implements ConversionService<SourcelResponseResultEntity, SourceTestResponseDto> {
    ...
    public SourceTestResponseDto convertForward(SourcelResponseResultEntity sourcelEntity) {
        ...
        return SourceTestResponseDto.builder()
                .updatedWhen(DateUtils.toString(sourcelEntity.getTouched(), DATEFORMAT))
                .requestKey(RequestKey.builder().key(sourcelEntity.getAccessKey().toString()).build())
                .lastSuccessfulFetch(DateUtils.toString(sourcelEntity.getLastSuccessfulFetch(), DATEFORMAT))
                .execTime(sourcelEntity.getLastExecTime())
                .histogram(toHistogram(sourcelEntity.getHistogramAsJson()))
                .state(SourceDto.State.valueOf(sourcelEntity.getState()))
                .build();
    }

    @Override
    public SourcelResponseResultEntity convertBackward(SourceTestResponseDto sourceDto) {
        SourcelResponseResultEntity.SourcelResponseResultEntityBuilder builder = SourcelResponseResultEntity.builder()
                .touched(new Date())
                .lastExecTime(sourceDto.getExecTime())
                .accessKey(sourceDto.getRequestKey() != null ? UUID.fromString(sourceDto.getRequestKey().getKey()) : UUID.randomUUID())
                .histogramAsJson(fromHistogram(sourceDto.getHistogram()))
                .state(sourceDto.getState().name());

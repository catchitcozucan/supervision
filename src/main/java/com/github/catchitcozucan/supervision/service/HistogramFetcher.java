...
import static com.github.catchitcozucan.supervision.api.SourceDto.State.POLLING_INITIALIZED;
import static com.github.catchitcozucan.supervision.exception.ErrorCodes.BAD_CONTENT_TYPE_IN_RESPONSE;
import static com.github.catchitcozucan.supervision.exception.ErrorCodes.BAD_RESPONSE_HTTP_CODE;
import static com.github.catchitcozucan.supervision.exception.ErrorCodes.NO_CONTENT_TYPE_IN_RESPONSE;
import static com.github.catchitcozucan.supervision.exception.ErrorCodes.PROXY_IS_UNKNOWN;
import static com.github.catchitcozucan.supervision.exception.ErrorCodes.REQUEST_GIVES_CONNECTION_REFUSED;
import static com.github.catchitcozucan.supervision.exception.ErrorCodes.REQUEST_GIVES_HTTP_CONECTION_TIMEOUT_EXCEPTION;
import static com.github.catchitcozucan.supervision.exception.ErrorCodes.REQUEST_GIVES_HTTP_TIMEOUT_EXCEPTION;
import static com.github.catchitcozucan.supervision.exception.ErrorCodes.REQUEST_GIVES_UNKNOWN_HOST_EXCEPTION;
...
public class HistogramFetcher {
    ...
    @Autowired
    private SourceConversionService sourceConversionService;
    @Autowired
    private GsonWrapper gson;
    @Autowired
    private SimpleHttpGetter simpleHttpGetter;

	private Object fetchLock = new Object();

	public enum AccessType {
		WRITE, TEST, READ_ONLY
	}

	public SourceTestResponseDto fetchHistogram(UUID requestKey, AccessType accessType) {
		synchronized (fetchLock) {
			SourceDto sourceDto = sourceConversionService.convertForward(sourceRepository.findFirstByAccessKey(requestKey).get());
			SourcelResponseResultEntity respons = fetchHistogramInner(sourceDto, requestKey, accessType);
			if (StringUtils.hasContents(respons.getState()) && !respons.getState().equals(AVAILABLE.name()) && !respons.getState().equals(POLLING_INITIALIZED.name())) {
	...
	public SourceTestResponseDto fetchHistogramViaUrlReadOnly(SourceDto sourceDto) {
		return sourceResponseResultConversionService.convertForward(fetchHistogramInner(sourceDto, UUID.fromString(sourceDto.getRequestKey().getKey()), AccessType.TEST));
	}

	private SourcelResponseResultEntity fetchHistogramInner(SourceDto sourceDto, UUID requestKey, AccessType accessType) {
		...
		} else if (accessType.equals(AccessType.READ_ONLY)) {
		...
		responseResultEntity.setState(POLLING_INITIALIZED.name());
		URI uri = null;
		if (StringUtils.hasContents(sourceDto.getAccessUrl())) {
			try {
		...
		if (uri != null && responseResultEntity.getState().equals(POLLING_INITIALIZED.name())) {
			...
			try {
				...
				if (accessType.equals(AccessType.WRITE)) {

...
public class HistogramFetcher {
    ...
    private SourcelResponseResultEntity fetchHistogramInner(SourceDto sourceDto, UUID requestKey, boolean readOnly) {
        ...
        if (!responseEntityOpt.isPresent()) {
            responseEntityOpt = Optional.of(SourcelResponseResultEntity.builder()
                    .accessKey(requestKey)
                    .touched(new Date())
                    .state(SourceDto.State.AVAILABLE.toString())
                    .build());
        }
            ...
            try {
                ...
                if (resp.executedOk()) {
                    try {
                        ...
                        responseResultEntity.setState(SourceDto.State.AVAILABLE.name());
                    } catch (Exception e) {
                ...
                Date newSucess = responseResultEntity.getState().equals(SourceDto.State.AVAILABLE.name()) && histogram != null ? new Date() : null;
                SourceTestResponseDto sourceM = SourceTestResponseDto.builder()
                        .histogram(histogram)
                        .execTime(SizeUtils.getFormattedMillisPrintoutFriendly(execTime))
                        .lastSuccessfulFetch(DateUtils.toString(newSucess, DATEFORMAT))
                        .state(SourceDto.State.valueOf(responseResultEntity.getState()))
                        .updatedWhen(newSucess != null ? DateUtils.toString(newSucess, DATEFORMAT) : DateUtils.toString(new Date(), DATEFORMAT))
                        .build();
